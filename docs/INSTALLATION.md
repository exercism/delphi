...
Delphi does not run natively in non-Windows based operating systems.  However, it is possible to install Delphi into a virtual machine which is running Windows.  If utilizing a Virtual Machine it is possible for Delphi to function.
...
With Delphi successfully installed and working correctly please follow these steps for retrieving and installing DUnitX if you find that your version of Delphi is lacking the DUnitX framework.
...
- The Starter Edition of Delphi doesn't allow the use of project experts.  DUnitX comes with an expert which will aid the student in creating a new DUnitX based project.  The expert creates the projects .DPR file.  The exercises in this track all include the .DPR file making the expert not necessary.  DUnitX otherwise has no issues with projects created in Delphi version starting with XE all the up to and including the Starter Edition of Berlin.
